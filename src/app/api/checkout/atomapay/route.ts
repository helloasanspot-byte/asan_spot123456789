...
export async function POST(req: Request) {
  try {
    ...
    // ۳. تولید آدرس هدایت سفارشی به ترمینال تایید و آپلود رسید آتوما‌پی (AtomaPay)
    const checkoutUrl = `/payment-confirm/atomapay?orderId=${orderId}`

    // ۴. بازگرداندن پاسخ موفقیت‌آمیز به همراه لینک هدایت
    return NextResponse.json({
      success: true,
      message: 'AtomaPay checkout route initialized successfully.',
      checkoutUrl: checkoutUrl
    })

  } catch (error: any) {
    console.error('⚠️ AtomaPay Checkout Fatal Error:', error)
